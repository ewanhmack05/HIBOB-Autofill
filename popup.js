...
function validateFields() {
	const hasProject = elProject.value.trim().length > 0;
	const hasTask = elTask.value.trim().length > 0;

	elTask.disabled = !hasProject;
	elFill.disabled = !(hasProject && hasTask);
	elSave.disabled = !(hasProject && hasTask);

	if (hasProject) elProject.classList.remove("error");
	if (hasTask) elTask.classList.remove("error");
}

function showErrors() {
	if (!elProject.value.trim()) elProject.classList.add("error");
	if (!elTask.value.trim()) elTask.classList.add("error");
}
...
async function saveSettings() {
	...
	if (tab?.id) {
		chrome.tabs.sendMessage(tab.id, { type: "TOGGLE_AUTOFILL", enabled: toggle });
	}
}
...
async function readFavourites() {
	const obj = await chrome.storage.sync.get(FAV_KEY);
	const favs = obj[FAV_KEY];
